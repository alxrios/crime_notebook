...
```
Again it seems that we have a variable composed of codes that correspond to different categories.
Let's check if the codes are the same as the ones in the variable Crm.Cd.
```{r}
range(crimeset$Crm.Cd)
range(crimeset$Crm.Cd.1, na.rm = T)
length(unique(crimeset$Crm.Cd))
...
```
Ranges are equal but the variable Crm.Cd.1 has two more unique values (without counting the ones that are missing) than the variable Crm.Cd. Let's check what codes are these.
```{r}
...
print(notFound)
```
The codes are 430 and 521. Since for this variable are two more codes than in Crm.Cd and we assumed than the variable with the descriptions (Crm.Cd.Desc) referred to the codes in Crm.Cd, we are going to be unable to associate descriptions to these two codes. Let's test now if the descriptions for Crm.Cd and for Crm.Cd.1 can be related, this way the meaning of this variable and the next ones (Crm.Cd.2, Crm.Cd.3, Crm.Cd.4) would be the ones described in Crm.Cd.Desc, and the codes with no match would be put to just missing.
```{r}
head(crimeset$Crm.Cd)
head(crimeset$Crm.Cd.1)
...
```
Both variables begin and end with the same codes, but as we have seen before, at some point they must differ. Let's search for observatins which take different values in both variables.
```{r}
sort(table(crimeset$Crm.Cd.1[which(crimeset$Crm.Cd != crimeset$Crm.Cd.1)]), decreasing = T)
```
So, for example, we can see that the observations that 474 observations that take the value 812 for the variable Crm.Cd.1 don't take the same value for the variable Crm.Cd. Let's see an example:
```{r}
...
```
So we can see that when the variable Crm.Cd.1 takes the value 812, 1494 observations of the variable take the same value, i.e. the same category, but in other cases they take one of six other values. Let's search for the corresponding descriptions and determine if they can be crimes charged to one unique event.
```{r}
```
